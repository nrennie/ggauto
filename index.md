...
x values. Since the output of
[`ggauto()`](http://nrennie.rbind.io/ggauto/reference/ggauto.md) is
...
in [`ggauto()`](http://nrennie.rbind.io/ggauto/reference/ggauto.md). The
latter is recommended as the arguments are used a little abnormally to
implement the styling. You can add markdown formatting into the title,
subtitle, or caption:
...
set.seed(123)
plot_data <- data.frame(
  v1 = sample(LETTERS[1:3], 200, replace = TRUE),
  v2 = sample(LETTERS[24:26], 200, replace = TRUE)
)
ggauto(plot_data$v1, plot_data$v2)
```
...
set.seed(123)
plot_data <- data.frame(
  v1 = sample(LETTERS[1:13], 200, replace = TRUE),
  v2 = sample(LETTERS[14:26], 200, replace = TRUE)
)
ggauto(plot_data$v1, plot_data$v2)
```
...
``` r
plot_data$v1 <- factor(plot_data$v1, levels = LETTERS[1:13])
ggauto(plot_data$v1, plot_data$v2)
```
