...
You can pass the column of data in as vectors:
...
penguins |>
  ...
  ggauto("island", "flipper_len", data = _)
```
...
mpg |> 
  ...
  dplyr::ungroup() |> 
  ggauto("cyl", "drv", "mean_hwy", data = _)
```
...
```{r}
mpg |> 
  ggauto("displ", "hwy", data = _)
```

To show the correlation between **two continuous variables**, split by a third **discrete** variable:

```{r}
mpg |> 
  dplyr::mutate(cyl = as.factor(cyl)) |> 
  ggauto("displ", "hwy", "cyl", data = _)
```
...
mpg |> 
  dplyr::mutate(cyl = as.factor(cyl)) |> 
  ggauto("displ", "hwy", "manufacturer", data = _)
```
